##################################################
## grepable simple channel stats version 1.0    ##
...
global chanlog
   if {[catch {set file [open $chanlog a 0600]} open_error] != 0} {
...
proc chanlog_rotate { minute hour day month year } {
global $chanlog
   putlog "\[\002chanlog\002\] Switching chanlog file from '$chanlog' to '$chanlog.yesterday'"
   if {[catch {exec mv -f $chanlog $chanlog.yesterday} error] != 0} {
      putlog "\[\002ERROR\002\] Could not move $chanlog to $chanlog.yesterday:  $error"
   }
   if {[catch {exec touch $chanlog} error] != 0} {
      putlog "\[\002ERROR\002\] Could not create new chanlog file:  $error"
   }
}
bind time - "00 00 * * *" chanlog_rotate
